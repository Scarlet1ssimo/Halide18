...
std::vector<Expr> DistributeVec::visit(const Reinterpret *op, unsigned num_chunks) {
    internal_assert(false) << "Reinterpret not supported for distribution\n";
...
std::vector<Expr> DistributeVec::visit(const Cast *op, unsigned num_chunks) {
    ...
    for (int i = start; i != end; i += step) {
        ...
        if (larger_size % bitvector_size != 0) {
            continue;
        }
        ...
        for (const auto &distrib_value : distributed_value) {
...
std::vector<Expr> DistributeVec::visit(const IntImm *op, unsigned num_chunks) {
    ...
    exprs.emplace_back(IntImm::make(op->type, op->value));
...
std::vector<Expr> DistributeVec::visit(const UIntImm *op, unsigned num_chunks) {
    ...
    exprs.emplace_back(UIntImm::make(op->type, op->value));
...
std::vector<Expr> DistributeVec::visit(const FloatImm *op, unsigned num_chunks) {
    ...
    exprs.emplace_back(FloatImm::make(op->type, op->value));
...
std::vector<Expr> DistributeVec::visit(const StringImm *op, unsigned num_chunks) {
    ...
    exprs.emplace_back(StringImm::make(op->value));
    ...
    if (op->is_intrinsic(Call::OP_NAME)) {                                                                  \
        ...
        for (unsigned bitvector_size : bitvector_sizes) {                                                   \
            ...
            if (operands_num_chunks != num_chunks) {                                                        \
                internal_assert(!exprs.empty()) << "Concat empty vectors for internal widen call nodes\n";  \
                debug(0) << "make concat "                                                                  \
                         << "call"                                                                          \
                         << "\n";                                                                           \
...
std::vector<Expr> DistributeVec::visit(const Call *op, unsigned num_chunks) {
    ...
    if (op->is_intrinsic(Call::rounding_mul_shift_right)) {
        ...
        for (unsigned bitvector_size : bitvector_sizes) {
            ...
            if (operands_num_bits % bitvector_size != 0) {
                continue;
            }
            ...
            if (operands_num_chunks != num_chunks) {
                internal_assert(!exprs.empty()) << "Concat empty vectors for rounding_mul_shift_right nodes\n";
                debug(0) << "make concat "
    ...
    if (op->is_intrinsic(Call::mul_shift_right)) {
        ...
        for (unsigned bitvector_size : bitvector_sizes) {
            ...
            // the results and then extract the slices accordingly.
            if (operands_num_chunks != num_chunks) {
                internal_assert(!exprs.empty()) << "Concat empty vectors for mul_shift_right nodes\n";
                debug(0) << "make concat "
                         << "mul_shift_right"
                         << "\n";
                Expr Combined = simplify(Shuffle::make_concat(exprs));
...
std::vector<Expr> DistributeVec::visit(const Shuffle *op, unsigned num_chunks) {
    ...
    if (op->is_interleave()) {
        ...
        internal_assert(!op->vectors.empty()) << "Distributed arguments require same number of operands\n";
        debug(0) << "make concat "
                 << "OrigShuff"
                 << "\n";
...
Stmt distribute_vector_exprs(const Stmt &S, unsigned bitwidth) {
...
// High level driver for distributing vector expressions
Stmt distribute_vector_exprs(const Stmt &S, std::vector<unsigned> bv_sizes, bool supports_sat_operations) {
    debug(0) << "Invoked distribute_vector_exprs with multiple bitwidths \n";
    Stmt inlined = substitute_in_all_lets(S);

    return ((DistributeVec(std::move(bv_sizes), supports_sat_operations).dispatch(inlined)));
}

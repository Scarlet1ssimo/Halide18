...
public:
    ...
    Stmt mutate(const Stmt &stmt) override {
        ...
        if (stmt.node_type() == IRNodeType::Store) {
            ...
            for (const auto *DS_IR : DeadStatements) {
...
public:
    ...
    IROptimizer(FuncValueBounds fvb, HydrideSupportedArchitecture _arch, std::set<const BaseExprNode *> &ms, int oid, std::string name)
        : arch(_arch), func_value_bounds(std::move(fvb)), mutated_exprs(ms), optimizer_id(oid), benchmark_name(std::move(name)) {
    }
    ...
    Expr mutate(const Expr &expr) override {
        ...
        if (arch == HydrideSupportedArchitecture::HVX || arch == HydrideSupportedArchitecture::ARM) {
            ...
            while (true) {
                ...
                if (equal(spec_expr, previous_expr)) {
                    break;
                }
...
private:
    ...
    public:
        ...
        Expr generate_call(const std::string &function_name, const Expr &x, std::map<std::string, Expr> &abstractions) {
    ...
    // synthesis
    Expr synthesize_impl(Expr spec_expr, const Expr &orig_expr);
...
public:
    ...
    std::string emit_buffer_id_map(const std::string &map_name) {
    ...
    std::string emit_symbolic_buffers_vector(const std::string &vector_name) {
    ...
    std::string emit_compile_to_llvm(const std::string &expr_name, const std::string &map_name, const std::string &call_name, const std::string &bitcode_path) {
    ...
    std::string emit_hydride_synthesis(const std::string &expr_name, size_t expr_depth, size_t VF, const std::string &id_map_name, const std::string &synth_log_path, const std::string &synth_log_name, const std::string &target) {
    ...
    std::string emit_interpret_expr(const std::string &expr_name) {
    ...
    std::string emit_assemble_result(const std::string &result_name, const std::string &expr_name, size_t lanes) {
        return "(define " + result_name + " (halide:assemble-bitvector " + emit_interpret_expr(expr_name) + " " + std::to_string(lanes) + ")" + ")";
    }

    std::string emit_write_synth_log_to_file(const std::string &fpath, const std::string &hash_name) {
        return "(save-synth-map \"" + fpath + "\" \"" + hash_name + "\" synth-log)";
    }
};

Expr IROptimizer::synthesize_impl(Expr spec_expr, const Expr &orig_expr) {
...
Stmt hydride_preprocess_hvx(const Stmt &s) {
...
Stmt hydride_preprocess_x86(const Stmt &s) {
...
Stmt hydride_optimize_hvx(FuncValueBounds fvb, const Stmt &s, std::set<const BaseExprNode *> &mutated_exprs) {
    ...
    auto Result = Hydride::IROptimizer(std::move(fvb), HydrideSupportedArchitecture::HVX, mutated_exprs, random_seed, name).mutate(distributed);
...
Stmt hydride_optimize_x86(FuncValueBounds fvb, const Stmt &s, std::set<const BaseExprNode *> &mutated_exprs) {
    ...
    const char *benchmark_name = getenv("HYDRIDE_BENCHMARK");
    std::string name = benchmark_name ? std::string(benchmark_name) : "hydride";
    auto Result = Hydride::IROptimizer(std::move(fvb), HydrideSupportedArchitecture::X86, mutated_exprs, random_seed, name).mutate(distributed);
...
Stmt optimize_x86_instructions_synthesis(Stmt s, const Target &t, FuncValueBounds fvb) {
    ...
    std::set<const BaseExprNode *> mutated_exprs;
    s = hydride_optimize_x86(std::move(fvb), s, mutated_exprs);
...
Stmt optimize_hexagon_instructions_synthesis(Stmt s, const Target &t, FuncValueBounds fvb) {
    ...
    debug(0) << "Input Statement to Compile through HVX:\n"
             << s << "\n";
    s = hydride_optimize_hvx(std::move(fvb), s, mutated_exprs);
...
Stmt optimize_arm_instructions_synthesis(Stmt s, const Target &t, FuncValueBounds fvb) {
    ...
    debug(0) << "Input Statement to Compile through HVX:\n"
             << s << "\n";
    s = hydride_optimize_arm(std::move(fvb), s, mutated_exprs);
